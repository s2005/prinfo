...
| uv.lock | Update | Regenerated by `uv lock` after the version bump |
...
- Bump the version to `0.4.0` in both `src/prinfo/__init__.py` and `pyproject.toml`, then run `uv lock`
  so `uv.lock` follows. The project bumps both files in one commit (see `57622a1` and `cbf07b8`);
  bumping only `__init__.py` leaves `prinfo --version` disagreeing with the installed package metadata.
